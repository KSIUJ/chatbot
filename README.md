...
- **Baza wektorowa:** ChromaDB (`PersistentClient`), dane w `dataset/vectorstore/`.
...
cp .env.example .env
# uzupełnij: MORDOR_COOKIE, USOS_CONSUMER_KEY/SECRET
```
...
# Zbudowanie/aktualizacja bazy wektorowej ze wszystkich źródeł
python -m src.backend.RAG.ingest.run_ingest

# Albo tylko wybrane źródło
...
│   │   └── RAG/                      # pipeline retrievalu
│   │       ├── encoder.py            # embeddingi (sentence-transformers)
│   │       ├── vectorstore.py        # ChromaDB
│   │       ├── retriever.py          # encoder + vectorstore
│   │       ├── context_builder.py    # interfejs dla warstwy LLM
│   │       └── ingest/               # wczytywanie danych źródłowych do vectorstore
│   ├── data/
│   │   ├── mordor/                   # pobieranie i przetwarzanie plików z Mordoru
│   │   ├── strony/                   # scraper stron wydziałowych, kół, Wikipedii
│   │   └── usos/                     # klient USOS API
│   └── frontend/                     # interfejs czatu (React + TypeScript)
├── tests/
│   ├── backend/RAG/                  # testy pipeline'u RAG
│   └── data/usos/                    # testy klienta USOS API
├── README.md
└── README.en.md
```
